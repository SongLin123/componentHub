<template>
  <el-dialog
    :title="(handelMode === 'add' && addDialogTitle) ||(handelMode === 'edit' && editDialogTitle) || (handelMode === 'remove' && removeDialogTitle)"
  ...
  >
    ...
    <el-form :model="form" :rules="rules" class="groupForm" ref="groupForm" v-if="hideDialogForm">
      <el-form-item
        :label="dialogFormlabel"
        :label-width="dialogFormlabelWidth+'px'"
        prop="name"
        v-if="handelMode !== 'remove'"
      >
        <el-input v-model="form.name" autocomplete="off"></el-input>
        <span class="device_tip">更改设备组名称后，系统将同步更改</span>
      </el-form-item>
      <el-form-item
        label="备注"
        label-width="80px"
        prop="marke"
        v-if="handelMode !== 'remove' && dialogFormMode === 'user'"
      >
        <el-input
          v-model="form.marke"
          autocomplete="off"
      ...
      <p class="remove_tip1" v-if="handelMode === 'remove'&& dialogFormMode === 'user'">您确定删除该组嘛？</p>
      <p class="remove_tip2" v-if="handelMode === 'remove' && dialogFormMode === 'user'">删除后不可恢复</p>
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      if (this.handelMode === 'remove') {
        this.$emit('removeGroup', { data: this.groupData, done: this.done })
        return
      }
