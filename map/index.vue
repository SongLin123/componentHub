...
export default {
    ...
    mounted() {
      if (this.mapImage.src) {
        this.createMap(this.mapImage)
      }
    },
    watch: {
      mapImage: {
        handler(val) {
          this.$nextTick(() => this.createMap(val))
        },
        deep: true
      },
      device: {
    ...
    methods: {
      ...
      // 载入轨迹线段顺序
      drawTrajectory() {
        const PATH = this.stmap.createPolylineByPoint(this.trajectoryLinesByPoints)
        PATH.draw()
        this.loading = false
      },
      // 获取底图上的对象的数据
